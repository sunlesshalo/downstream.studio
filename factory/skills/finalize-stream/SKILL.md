...
2. **IMPORTANT:** Add the story title (from `stream.title`) as `<h1>` at the start of the FIRST section
3. For each section, create a content component
4. Apply text_type and dialogue_markers for formatting
...
export function IntroContent() {
  ...
  // FIRST section includes the story title
  return (
    <>
      <h1>{stream.title}</h1>
...
export function EpilogueContent() {
  ...
  // LAST section includes the CTA button
  return (
    <>
      {/* Exact text from sections[N].text_content */}

      {/* CTA Button - Add to the LAST section */}
...
- Add the "Beszéljünk" button to the LAST section of every demo stream
- Links to: https://cal.com/ferencz-csuszner/30-perces-online-beszelgetes
...
Use colors from `production.json` visual_direction.color_palette:
...
:root {
  --ds-color-background: {color_palette.background};
  --ds-color-text: #3d3d3d;  /* dark text for light backgrounds, light for dark */
  --ds-color-accent: {color_palette.primary};
  --ds-color-muted: {color_palette.accent};
}
...
html, body {
  ...
  min-height: 100%;
  width: 100%;
  overscroll-behavior: none;  /* IMPORTANT: Prevents rubber band effect */
}
...
/* Prevent rubber band effect on text column */
